...
** reform(indir, type)
*** 有効シート名取得
#+BEGIN_SRC R :results silent :exports code :tangle reformr/R/get_sheet2load.R
  get_sheet2load <- function(x) {
    re_omit  <- "^(?!.*0000)(?!体長)(?!Sheet)"
    re_match <- "(0[1-9][0-9]{2}|1[0-2][0-9]{2})(\\([0-9]+\\))?"
    regex <- paste0(re_omit, re_match)
    match <- x %>%
      stringr::str_match(regex)
    out <- match[,1] %>%         # '1' points column which contains full match
      na.omit()
    as.vector(out)
  }
#+END_SRC
