...
test_that("extract_a_block() returns block from df", {
  data  <- data.frame(a = rep(c("foo", "bar", "baz", "bum"), 4),
                     b = 1:16, c = 11:26, d = 21:36, stringsAsFactors = FALSE)
  data2 <- extract_a_block(pos.key = 1, find_from = 1, direction = "row",
  ...
  data2 <- extract_a_block(pos.key = 1, find_from = 1, direction = "col",
                           df = data, offset = c(0, 0), dim = c(2, 2))
...
test_that("extract_blocks() returns row blocks", {
  ...
  data2 <- extract_blocks(data, "foo", col = 1, dim = c(3, 3))
  ...
  data2 <- extract_blocks(data, "foo", col = 1, offset = c(2, 1), dim = c(3, 2))
